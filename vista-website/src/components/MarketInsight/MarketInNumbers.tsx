...
function MarketInNumbers() {
  return (
    <div className="grid sm:grid-cols-3 w-full h-full place-items-center select-none">
      {MarketInNumbersCode.map((item, index) => (
        <div className="dark:text-white text-heading flex justify-between items-center gap-5" key={index}>
          ...
          <div className="inline-flex flex-col">
            ...
            <span className="text-sm dark:text-[#fef] text-heading sm:w-2/3 ">{item.description.trim()}</span>
          </div>
        </div>
      ))}
    </div>
