  ...
  return (
    ...
    <h3 className="text-2xl font-bold capitalize text-heading sm:text-4xl ">
      ...
      {content.map((benefit, index) => (
        <div key={index} className="flex flex-col cursor-pointer items-center space-y-2 text-center bg-white shadow-md overflow-hidden hover:bg-accent/50 group transition rounded-xl p-5 max-w-96">
          ...
          <p className="text-sm text-heading group-hover:text-white">{benefit.content}</p>
        </div>
      ))}
      </div>
    </SectionLayout>
